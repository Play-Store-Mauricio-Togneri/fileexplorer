 ...
 *
 * The volume sizes are injected rather than statted. Reaching any of the above through the real
 ...
 * a single one of these assertions had anything to look at.
 */
class AndroidStorageSourceTest {

    @Test
    fun `getStorages reports nothing when the framework reports no volume`() = runTest {
        val source = AndroidStorageSource(contextWith(), allVolumesReadable)
    ...
    fun `getStorages resolves an app directory to the volume it sits on`() = runTest {
        val source = AndroidStorageSource(contextWith(appDirOn(PRIMARY)), allVolumesReadable)
    ...
    fun `getStorages reports an sd card alongside internal storage`() = runTest {
        val source = AndroidStorageSource(
    ...
    fun `getStorages collapses entries that resolve to the same volume`() = runTest {
        ...
        val source = AndroidStorageSource(
            contextWith(appDirOn(PRIMARY), appDirOn(PRIMARY)),
            allVolumesReadable
        )
    ...
    fun `getStorages ignores a volume the framework reported as null`() = runTest {
        ...
        val source = AndroidStorageSource(
            contextWith(appDirOn(PRIMARY), null, appDirOn(SD_CARD)),
            allVolumesReadable
        )
    ...
    @Test
    fun `getStorages numbers each kind of volume independently`() = runTest {
        val source = AndroidStorageSource(
            contextWith(
                appDirOn(PRIMARY),
    ...
    @Test
    fun `getStorages numbers nothing when a kind of volume has only one member`() = runTest {
        val source = AndroidStorageSource(
    ...
    fun `getStorages keeps a directory that does not end in the app directory suffix`() = runTest {
        ...
        val source = AndroidStorageSource(contextWith(unexpected), allVolumesReadable)
