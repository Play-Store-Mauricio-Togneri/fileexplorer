...
Severity counts: 3 High, 13 Medium, 14 Low (30 findings).
...
**Partial.** Unmet completeness conditions:

- Verification was **static only** — no on-device/emulator execution. Three findings rely on
  documented platform behavior rather than runtime reproduction: the SQLite corruption-handler
  deletion (`DefaultDatabaseErrorHandler.onCorruption`), the `OutOfMemoryError` escape paths, and
  the DataStore corruption crash. No disposable verification workspace was constructed because no
  mutating commands were run.
- File and flow inspection was performed by parallel discovery agents over a deterministic partition
  plus first-party re-verification of the high-risk and all reported files; a few agent inspection
  lists noted partial coverage on incidental cross-referenced files, and an exhaustive per-file
  second human read was not performed for every one of the 141 files.
- A formal "final discovery pass yielding zero new candidates" was approximated (convergence across
  11 agents + targeted re-reads) but not iterated to a guaranteed fixpoint.

All other workflow steps were completed: deterministic full-file inventory, high-risk surface pass (
file ops, archive, parsers, persistence, intents, build/release), independent re-verification with a
refutation attempt per finding, and a clean final worktree comparison.

### Initial Worktree Snapshot

- Repository: `/home/max/Repositories/personal/fileexplorer-new`
- Branch: `main`; Commit: `b213df442e0fe4fc1f2c0b6bf60622dbcbf5761e`
- Initial state: clean — 0 modified tracked paths, 0 untracked paths; staged and unstaged diffs both
  empty (`e3b0c442…855`).
- Tracked-manifest digest (338 files, sha256 of `sha256sum` list):
  `c19aabc8a09c990a9fdbd881bb019ee100b52104b09be57e37863ace521c6b59`.
- Ignored paths: `gradle.properties`, `keystore.properties`, `/build`, `/app/build`, `.gradle`,
  `/.idea`, `local.properties`, `*.iml` (excluded from the preservation guarantee; not modified —
  audit was read-only).
- Pre-existing `BUG_FINDINGS.md`: none (no replacement-approval needed).
- Final comparison: worktree re-hashed at completion — manifest digest identical, status clean,
  diffs empty. Only `BUG_FINDINGS.md` (this file, previously untracked/non-existent) was created.
  External snapshot metadata in `/tmp/apocalypse-audit-fileexplorer-snap` to be removed.

### Audit Coverage

- **Included first-party files:** 141 Kotlin main-source files + 1 `AndroidManifest.xml` + 3
  `res/xml` config files + `strings.xml` × 21 locales + 4 Gradle/version-catalog files +
  `proguard-rules.pro` + 3 shell scripts.
- **Inspected:** all 141 Kotlin files (assigned 1:1 to 11 discovery agents via a verified
  partition — `included = inspected (141) + skipped (0)`), plus the manifest, `provider_paths.xml`/
  `backup_rules.xml`/`data_extraction_rules.xml`, all 21 `strings.xml`, the 4 build files,
  `proguard-rules.pro`, and the 3 scripts. ~32 central files were additionally re-read in full by
  the synthesizer for verification.
- **Skipped:** 0 first-party source files.
- **Excluded (out of scope):** `app/build`, `build`, `.gradle`, `.idea` (generated/IDE);
  `google-services.json` (config — keys are non-secret and shipped); test sources were read as
  evidence of intended contracts, not audited for their own defects.
- **Deterministic partition check:** the 141-file partition reconciled exactly (
  `141 assigned = 141 unique`, 0 gaps, 0 duplicates) against
  `git ls-files 'app/src/main/java/**/*.kt'`.
- **Meaningful flows (representative inventory, all traced):**

| ID   | Flow (entry point)                                  | Material result / side effect     | Status |
|------|-----------------------------------------------------|-----------------------------------|--------|
| F-01 | Launch / permission gate (MainActivity → NavGraph)  | Home or permission screen         | traced |
| F-02 | List a folder (FolderViewModel.loadFiles)           | files + child counts              | traced |
| F-03 | Copy/Move via picker (executeOperation → copyFiles) | filesystem copy/move + MediaStore | traced |
| F-04 | Delete (small & progress paths)                     | filesystem delete + MediaStore    | traced |
| F-05 | Rename (renameRegular/renameCaseOnly)               | filesystem rename + MediaStore    | traced |
| F-06 | Create folder (folder + picker)                     | mkdir                             | traced |
| F-07 | Compress (compressFiles)                            | zip output + scan                 | traced |
| F-08 | Uncompress (UncompressHandler → uncompressFile)     | extracted files + scan            | traced |
| F-09 | Search (SearchViewModel → searchFilesStreaming)     | streamed results                  | traced |
| F-10 | Open file / share / install APK (IntentUtil)        | intent + recents + analytics      | traced |
| F-11 | Item Info + metadata extraction (12 extractors)     | metadata + folder size            | traced |
| F-12 | Recent files add/remove/clear                       | DataStore JSON                    | traced |
| F-13 | Preferences/theme/sort/locations/badges             | DataStore                         | traced |
| F-14 | Text/Image viewers                                  | bounded read / Coil decode        | traced |
| F-15 | Thumbnail load pipeline (Coil fetchers)             | bitmap                            | traced |
| F-16 | Release build & tester distribution (scripts)       | AAB/APK artifacts                 | traced |

Total enumerated meaningful flows: 16; traced: 16; skipped: 0. Coverage invariants hold (
`141 = 141 + 0`; `16 = 16 + 0`). All taxonomy categories (Tier A runtime correctness, Tier B
contracts/data/security, Tier C broader anomalies) were inspected; findings span logic,
null/numeric, boundary/encoding, error-handling, concurrency, resource/state, contract, validation,
config-parity, security, and API-misuse categories.

### Candidate Dispositions

35 candidates; 30 findings, 4 merged, 1 refuted, 0 unverified (`35 = 30 + 4 + 1 + 0`; reported
findings = findings = 30).

| ID    | Primary location                                              | Category             | Disposition                             |
|-------|---------------------------------------------------------------|----------------------|-----------------------------------------|
| C-001 | FileRepository.kt:295,327                                     | error-handling       | Finding (move failure-as-success)       |
| C-002 | FileRepository.kt:318                                         | error-handling       | Finding (copy partial dest)             |
| C-003 | FileRepository.kt:544-600                                     | error-handling       | Finding (extract partial files)         |
| C-004 | FolderViewModel.kt:392-399                                    | contract-mismatch    | Finding (MediaStore drift)              |
| C-005 | FileRepository.kt:221,243                                     | logic                | Finding (delete progress count)         |
| C-006 | FileRepository.kt:283-286                                     | contract-mismatch    | Finding (symlink copy/move)             |
| C-007 | FileRepository.kt:528                                         | numeric overflow     | Finding (size-sum overflow)             |
| C-008 | FileRepository.kt:545-590                                     | concurrency          | Merged → C-011                          |
| C-009 | FolderViewModel.kt:345-362                                    | error-handling       | Finding (StatFs crash)                  |
| C-010 | FolderViewModel.kt:576                                        | error-handling       | Merged → C-004                          |
| C-011 | FileRepository.kt:386-419                                     | concurrency          | Finding (missing cancellation)          |
| C-012 | HomeViewModel.kt:177-201                                      | concurrency          | Finding (load race)                     |
| C-013 | PickerViewModel.kt:153-191                                    | concurrency          | Finding (main-thread canWrite)          |
| C-014 | PickerViewModel.kt:203-216                                    | error-handling       | Finding (silent createFolder)           |
| C-015 | PdfThumbnailFetcher.kt:45-50                                  | numeric/OOM          | Finding (unbounded dims)                |
| C-016 | VideoThumbnailFetcher.kt:38                                   | api-misuse/OOM       | Finding (ignores size)                  |
| C-017 | EpubThumbnailFetcher.kt:36                                    | api-misuse/OOM       | Merged → C-016                          |
| C-018 | ImageMetadataExtractor.kt:98                                  | numeric              | Finding (focal length NaN)              |
| C-019 | SqliteMetadataExtractor.kt:16                                 | security (data loss) | Finding (corruption delete)             |
| C-020 | OfficeMetadataExtractor.kt:18; EpubMetadataExtractor.kt:19,35 | error-handling/OOM   | Finding (metadata OOM crash)            |
| C-021 | CsvMetadataExtractor.kt:18                                    | boundary/encoding    | Finding (CSV miscount)                  |
| C-022 | TextFilePreview.kt:43                                         | boundary/encoding    | Refuted (documented UTF-8-only preview) |
| C-023 | MimeTypeUtil.kt:135-138                                       | config-parity        | Finding (svgz misclassified)            |
| C-024 | RecentFilesRepository.kt:28-50                                | concurrency          | Finding (recents RMW race)              |
| C-025 | PreferencesRepository.kt:32-48                                | error-handling       | Finding (DataStore crash)               |
| C-026 | FileRepository.kt:162-202                                     | logic (data loss)    | Finding (rename overwrite)              |
| C-027 | FileNameValidator.kt:20-24                                    | validation           | Finding (validator gaps)                |
| C-028 | FileNameValidator.kt:20-24                                    | boundary             | Merged → C-027                          |
| C-029 | ItemInfoViewModel.kt:261-269                                  | concurrency/resource | Finding (folder-size symlink)           |
| C-030 | SwipeableFileListItem.kt:64,120                               | state/lifecycle      | Finding (swipe offset)                  |
| C-031 | CreateFolderDialog.kt:40; RenameDialog.kt:51                  | state/lifecycle      | Finding (non-saveable state)            |
| C-032 | app/build.gradle.kts:30-52                                    | release/build        | Finding (unsigned release)              |
| C-033 | gradle.properties                                             | config-parity        | Finding (missing template)              |
| C-034 | provider_paths.xml:3-8                                        | security             | Finding (over-broad provider)           |
| C-035 | scripts/distribute.sh:10-17                                   | release process      | Finding (debug build / jq)              |

### Verification Performed

- Read in full and traced: `FileRepository.kt`, `FolderViewModel.kt`, `HomeViewModel.kt`,
  `PickerViewModel.kt`, `ItemInfoViewModel.kt`, `SqliteMetadataExtractor.kt`,
  `PdfThumbnailFetcher.kt`, `VideoThumbnailFetcher.kt`, `Office/Epub/Image/CsvMetadataExtractor.kt`,
  `TextFilePreview.kt`, `ZipMetadataExtractor.kt`, `UncompressHandler.kt`, `IntentUtil.kt`,
  `MediaStoreUtil.kt`, `RenameDialog.kt`, `SwipeableFileListItem.kt`, `FileItem.kt`,
  `FileNameValidator.kt`, `PreferencesRepository.kt`, `DataStorePreferencesSource.kt`,
  `DataStoreRecentFilesSource.kt`, `RecentFilesRepository.kt`, `FileExplorerApplication.kt`,
  `MainActivity.kt`, `MimeTypeUtil.kt` (relevant), `app/build.gradle.kts`, `AndroidManifest.xml`,
  `provider_paths.xml`, `scripts/build.sh`, `scripts/distribute.sh`.
- Cross-checked the test suite (`app/src/test`, `app/src/androidTest`) to confirm contested
  behaviors are not pinned: the rename-overwrite path is masked by a JVM-only pre-O test; copy/move
  IO-failure cleanup, SQLite corruption, and folder-size symlink handling have no covering test.
- Static refutation per finding (guards/contracts/framework behavior); refuted candidates and
  refuted angles recorded.
- Worktree integrity verified before/after via `git status`, staged/unstaged diff hashes, and a full
  re-hashed tracked manifest compared to the snapshot (identical).
- No commands expected to mutate files or external state were executed; no tests/builds were run (
  static audit).

### Exclusions and Limitations

- **No runtime reproduction.** Findings C-019 (SQLite delete-on-corruption), C-015/C-016/C-020 (OOM
  behavior and whether Coil swallows the `Throwable`), and C-025 (DataStore corruption crash) are
  reasoned from documented Android/Coil/JDK behavior, not executed. Confidence ratings reflect this.
- **Coil internals not read:** whether Coil 2.7.0 converts a fetcher `OutOfMemoryError` to a failed
  request (no app crash) vs. propagates it was assumed (it generally catches `Throwable`); this only
  affects whether the thumbnail OOMs hard-crash, not the underlying unbounded-allocation defect.
- **Refuted (examined, not defects):** destination-inside-source recursion (picker
  `validateDestination` + disabled Confirm); ZIP slip (canonical-path prefix check with separator);
  SQL injection in SQLite metadata (quoted identifiers); XXE (no `FEATURE_PROCESS_DOCDECL`);
  resource leaks in extractors (`.use`/`finally` on all paths); enum decode crash (
  `entries.find { } ?: default`); duplicate DataStore instances (three distinct names, single
  delegates); exported `MainActivity` processing untrusted intent data (it ignores intent
  extras/data); localization parity and `%`-format-arg/plural-quantity integrity across all 20
  locales (verified clean by XML/format-token diff). `TextFilePreview` UTF-8-only decoding (C-022)
  is documented intended behavior, so reported as a limitation rather than a defect (it does produce
  mojibake for UTF-16/Latin-1 text previews).
- **Not deeply analyzed:** process-death / `SavedStateHandle` restoration (ViewModels hold state
  in-memory; mid-operation process death cancels pending ops, judged acceptable); dependency CVE
  status (no network/SCA run — versions are internally coherent and current as of 2026, none flagged
  from knowledge).

### Summary

- **By severity:** High 3, Medium 13, Low 14 (30 total).
- **By confidence:** High 9, Medium 20, Low 1.

| Severity  | High conf | Medium conf | Low conf | Total  |
|-----------|-----------|-------------|----------|--------|
| Critical  | 0         | 0           | 0        | 0      |
| High      | 1         | 2           | 0        | 3      |
| Medium    | 6         | 7           | 0        | 13     |
...
- **Most important findings:** (1) rename silently overwrites an existing hidden file (data loss); (
  2) viewing Info on a corrupt `.db` deletes the user's file; (3) a corrupt preferences file
  crash-loops the app at startup; plus move-failure-reported-as-success and crafted-document OOM
  crashes.
- **Affected files (unique, 24):** `data/repository/FileRepository.kt`,
  `ui/screens/folder/FolderViewModel.kt`, `ui/screens/home/HomeViewModel.kt`,
  `ui/screens/picker/PickerViewModel.kt`, `ui/screens/iteminfo/ItemInfoViewModel.kt`,
  `data/util/SqliteMetadataExtractor.kt`, `data/util/OfficeMetadataExtractor.kt`,
  `data/util/EpubMetadataExtractor.kt`, `data/util/PdfThumbnailFetcher.kt`,
  `data/util/VideoThumbnailFetcher.kt`, `data/util/EpubThumbnailFetcher.kt`,
  `data/util/ImageMetadataExtractor.kt`, `data/util/CsvMetadataExtractor.kt`,
  `data/util/MimeTypeUtil.kt`, `data/repository/PreferencesRepository.kt`,
  `data/repository/RecentFilesRepository.kt`, `util/FileNameValidator.kt`,
  `util/UncompressHandler.kt`, `ui/components/RenameDialog.kt`,
  `ui/components/CreateFolderDialog.kt`, `ui/components/SwipeableFileListItem.kt`,
  `app/build.gradle.kts`, `app/src/main/res/xml/provider_paths.xml`, `scripts/distribute.sh` (+
  `scripts/build.sh`, `gradle.properties`).
